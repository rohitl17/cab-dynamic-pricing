...
#		lyft_train = pd.read_csv('./training_testing_data/lyft_train_mlr.csv')
		lyft_train = pd.DataFrame({'source_lat':[42.3600825],'source_long':[-71.0588801],'dest_lat':[42.3518662],'dest_long':[-71.0642623],
    	'distance':[1.0],'surge_multiplier':[1.0],'Lux':[1.0],'Lux Black':[0],'Lux Black XL':[0],'Lyft':[0],'Lyft XL':[0],'Shared':[0],
    	'lyft_price':[7.0]})
...
#		lyft_test = pd.read_csv('./training_testing_data/lyft_test_mlr.csv')
		lyft_test = pd.DataFrame({'source_lat':[42.3600825],'source_long':[-71.0588801],'dest_lat':[42.3518662],'dest_long':[-71.0642623],
    	'distance':[1.0],'surge_multiplier':[1.0],'Lux':[1.0],'Lux Black':[0],'Lux Black XL':[0],'Lyft':[0],'Lyft XL':[0],'Shared':[0],
    	'lyft_price':[7.0]})
	...
	def test_uber_training(self):
		...
		uber_test = pd.DataFrame({'source_lat':[42.3600825],'source_long':[-71.0588801],'dest_lat':[42.3518662],'dest_long':[-71.0642623],'distance':[1.0],'surge_multiplier':[1.0],'Black':[0],'Black SUV':[0],'UberPool':[0],'UberX':[1.0],'UberXL':[0],'WAV':[0],'uber_price':[12.0]})
