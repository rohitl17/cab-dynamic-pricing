...
class PricePredictModel:
    def __init__(self):
        ...
        self.uber_train = pd.read_csv('./training_testing_data/uber_train_mlr.csv')
        self.lyft_train = pd.read_csv('./training_testing_data/lyft_train_mlr.csv')
        
        self.uber_test = pd.read_csv('./training_testing_data/uber_test_mlr.csv')
        self.lyft_test = pd.read_csv('./training_testing_data/lyft_test_mlr.csv')
        
        Xuber_train = self.uber_train[['source_lat','source_long', 'dest_lat', 'dest_long','distance','surge_multiplier','Black','Black SUV', 'UberPool', 'UberX', 'UberXL', 'WAV']]
        Yuber_train = self.uber_train['price']
        
        Xuber_test = self.uber_test[['source_lat','source_long', 'dest_lat', 'dest_long','distance','surge_multiplier','Black','Black SUV', 'UberPool', 'UberX', 'UberXL', 'WAV']]
        Yuber_test = self.uber_test['price']
        
        Xlyft_train = self.lyft_train[['source_lat','source_long', 'dest_lat', 'dest_long','distance','surge_multiplier','Lux','Lux Black', 'Lux Black XL', 'Lyft', 'Lyft XL', 'Shared']]
        Ylyft_train = self.lyft_train['price']
        
        Xlyft_test = self.lyft_test[['source_lat','source_long', 'dest_lat', 'dest_long','distance','surge_multiplier','Lux','Lux Black', 'Lux Black XL', 'Lyft', 'Lyft XL', 'Shared']]
    ...
    def lyft_train_test(self,Xlyft_train,Ylyft_train,Xlyft_test,Ylyft_test,result_lyft):
        ...
        result_lyft.tocsv('./result/lyft_price_mlr' + timestamp + '.csv')
